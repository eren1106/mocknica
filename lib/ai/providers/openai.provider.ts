...
export class OpenAIProvider extends AIProvider {
  ...
  async getAvailableModels(): Promise<AIModel[]> {
    try {
      ...
      // Use the official SDK to get models
      const response = await client.models.list();
      
      // Filter for GPT models and map to our format
      const gptModels = response.data
        .filter(model => model.id.includes('gpt') || model.id.includes('text-davinci'))
        .map(model => ({
          id: model.id,
          name: model.id,
          provider: AIProviderType.OPENAI,
      ...
      // If no models found via API, return empty
      if (gptModels.length === 0) return [];
  ...
  //   if (modelId.includes('gpt-4o')) return 'Latest multimodal model with vision capabilities';
  //   if (modelId.includes('gpt-4')) return 'Advanced reasoning for complex tasks';
